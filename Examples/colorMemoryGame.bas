...
[newRound]
    REM Lisää satunnainen väri sekvenssiin
    LET newColor$ = RND(NUM_COLORS#)
    seq$(round$) = newColor$
    round$ = round$ + 1

    REM --- Näytä ympyrä värillä ---
    LET ci$ = newColor$
    ...
    REM Näytä 5 sekuntia
    SLEEP 5000

    REM --- clear screen and ask colors so far
    SCREENLOCK ON
    LINE (0,0)-(639,479), 0, BF
